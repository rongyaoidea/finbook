...
impl BalanceTableView {
    ...
    fn reload(&mut self, ctx: &mut AppCtx<'_>) {
        let key = format!(
            "{}|{}|{}|{}|{}|{}|{}",
            self.from, self.to, self.level, self.leaf_only, self.non_zero, self.code_from, self.code_to
        );
    ...
    pub fn show(&mut self, ctx: &mut AppCtx<'_>, ui: &mut Ui) {
        ...
        widgets::page_header(ui, "科目余额表", |ui| {
            ui.label(RichText::new(format!("{} 行", self.rows.len())).weak());
        });
